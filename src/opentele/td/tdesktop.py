...
class TDesktop(BaseObject):
    ...
    def __init__(
        ...
        self.__keyFile = (
            keyFile if (keyFile != None) else TDesktop.kDefaultKeyFile
        )
    ...
    def LoadTData(
        ...
        Expects(
            self.isLoaded(), "Failed to load? Something went seriously wrong"
        )
    ...
    def SaveTData(
        ...
        Expects(
            basePath != None and basePath != "",
            "No folder provided to save tdata",
        )
        ...
        try:
            ...
            raise TDataSaveFailed(
                "Could not save tdata, something went wrong"
            ) from e
    ...
    def __loadFromTData(self) -> None:
        ...
        Expects(
            count > 0,
            "accountsCount is zero, the data might has been corrupted",
        )
