...
class Storage(BaseObject):
    class ReadSettingsContext(BaseObject):  # pragma: no cover
        def __init__(self) -> None:
            self.fallbackConfigLegacyDcOptions: td.MTP.DcOptions = (
                td.MTP.DcOptions(td.MTP.Environment.Production)
            )
    ...
    class FileWriteDescriptor(BaseObject):
        def __init__(
            self, fileName: str, basePath: str, sync: bool = False
        ) -> None:
    ...
    @staticmethod
    def PrepareEncrypted(
        data: EncryptedDescriptor, key: td.AuthKey
    ) -> QByteArray:
        data.finish()
        ...
        encrypted.resize(
            0x10 + fullSize
        )  # 128bit of sha1 - key128, sizeof(data), data
    ...
    def CreateLocalKey(
        ...
        return td.AuthKey(
            hashlib.pbkdf2_hmac(
                "sha512", hashKey.digest(), salt, iterationsCount, 256
            )
        )
    ...
    @staticmethod
    def DecryptLocal(
        encrypted: QByteArray, authKey: td.AuthKey
    ) -> EncryptedDescriptor:
        encryptedSize = encrypted.size()
        if (encryptedSize <= 16) or (encryptedSize & 0x0F):
            raise TDataBadEncryptedDataSize(
                "Bad encrypted part size: {encryptedSize}"
            )
        ...
        decrypted = Storage.aesDecryptLocal(
            encryptedData, authKey, encryptedKey
        )
