...
class MapData(BaseObject):  # nocov
    ...
    def read(self, localKey: td.AuthKey, legacyPasscode: QByteArray) -> None:
        ...
        ExpectStreamStatus(
            mapData.stream, "Could not stream data from mapData"
        )
        ...
        if not localKey:
            ...
            try:
                keyData = td.Storage.DecryptLocal(
                    legacyKeyEncrypted, legacyPasscodeKey
                )
            except OpenTeleException as e:
        ...
        while not is_finished and not map.stream.atEnd():
            ...
            ExpectStreamStatus(
                map.stream, "Could not stream data from mapData "
            )
    ...
    def prepareToWrite(self) -> td.Storage.EncryptedDescriptor:
        ...
        if len(self._draftsMap) > 0:
            mapSize += (
                sizeof(uint32) * 2 + len(self._draftsMap) * sizeof(uint64) * 2
            )
        if len(self._draftCursorsMap) > 0:
            mapSize += (
                sizeof(uint32) * 2
                + len(self._draftCursorsMap) * sizeof(uint64) * 2
            )
        ...
        if (
            ...
            or self._recentMasksKey
            or self._archivedMasksKey
        ):
    ...
    def readMtpConfig(self) -> td.MTP.Config:
        ...
        Expects(
            ...
            AccountAuthKeyNotFound(
                "The localKey has not been initialized yet"
            ),
        )

        try:
            file = td.Storage.ReadEncryptedFile(
                "config", self.basePath, self.localKey
            )  # type: ignore
            serialized = QByteArray()
            file.stream >> serialized

            ExpectStreamStatus(
                file.stream, "Could not stream data from MtpConfig"
            )
...
class Account(BaseObject):
    ...
    def __init__(
        ...
        self.__keyFile = (
            keyFile if (keyFile != None) else td.TDesktop.kDefaultKeyFile
        )
        ...
        self._local = StorageAccount(
            self,
            self.basePath,
            td.Storage.ComposeDataString(self.__keyFile, index),
        )
        self.index = index
    ...
    def _setMtpAuthorization(self, serialized: QByteArray):
        ...
        def readKeys(keys: typing.List[td.AuthKey]):
            ...
            for i in range(key_count):
                ...
                keys.append(
                    td.AuthKey.FromStream(
                        stream, td.AuthKeyType.ReadFromFile, dcId
                    )
                )
