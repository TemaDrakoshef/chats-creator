...
class CustomInitConnectionRequest(functions.InitConnectionRequest):
    def __init__(
        ...
        if data != None:
            ...
            self.device_model = (
                data.device_model if data.device_model else device_model
            )
            self.system_version = (
                data.system_version if data.system_version else system_version
            )
            self.app_version = (
                data.app_version if data.app_version else app_version
            )
            self.system_lang_code = (
                data.system_lang_code
                if data.system_lang_code
                else system_lang_code
            )
...
class TelegramClient(telethon.TelegramClient, BaseObject):
    ...
    async def GetCurrentSession(self) -> Optional[types.Authorization]:
        ...
        return (
            next(
                (auth for auth in results.authorizations if auth.current), None
            )
            if results != None
            else None
        )
    ...
    async def PrintSessions(
        ...
        """
        if (sessions == None) or not isinstance(
            sessions, types.account.Authorizations
        ):
            sessions = await self.GetSessions()
        ...
        for attempt in range(request_retries):  # nocov
            try:
                ...
                if isinstance(qr_login._resp, types.auth.LoginTokenSuccess):
                    coro = newClient._on_login(
                        qr_login._resp.authorization.user
                    )
            ...
            warnings.warn(
                "\nQRLoginToNewClient attemp {} failed. Retrying..".format(
                    attempt + 1
                )
            )
...
def PrettyTable(table: List[Dict[str, Any]], addSplit: List[int] = []):
    ...
    splitter = "+".join(
        ("-" * (spaces + 2)) for label, spaces in padding.items()
    )
    rows = []
    for row in table:
        rows.append(
            "|".join(
                addpadding(row[label], spaces + 2)
                for label, spaces in padding.items()
            )
        )
