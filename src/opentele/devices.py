...
class SystemInfo(BaseObject):
    ...
    @classmethod
    def RandomDevice(
        cls: Type[SystemInfo], unique_id: str = None
    ) -> DeviceInfo:
    ...
    def _strtohashid(cls, unique_id: str = None):
        ...
        byteid = (
            os.urandom(32) if unique_id == None else unique_id.encode("utf-8")
        )
            ...
            for id_model in cls.device_models:
                ...
                for model_name in cls.device_models[id_model]:
                    ...
                    for major in available_versions:
                        for minor, patches in cls.system_versions[
                            major
                        ].items():
                            if len(patches) == 0:
                                results.append(
                                    DeviceInfo(
                                        device_model, f"{major}.{minor}"
                                    )
                                )
                            else:
